...
mean_plot <- ggplot() +
  geom_line(data = mean_data, aes(x = SampleSize, y = SampleMean, color = Line, group = Line), size = 0.5, colour = "#fdd0a2") +
...
variance_plot <- ggplot() +
  geom_line(data = variance_data, aes(x = SampleSize, y = SampleVariance, color = Line, group = Line), size = 0.5, colour = "#cab2d6") +
  ...
  geom_hline(yintercept = 1, linetype = "dashed", size = 1.5) +
  labs(title = " ", x = "sample size", y = "variance") +
  scale_y_continuous(breaks = c(0, 1, 2), limits = c(0, 2), minor_breaks = NULL) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 50),
